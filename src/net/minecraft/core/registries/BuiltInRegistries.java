...
public class BuiltInRegistries {
   ...
   private static <T> Registry<T> registerSimple(ResourceKey<? extends Registry<T>> pKey, BuiltInRegistries.RegistryBootstrap<T> pBootstrap) {
      return registerSimple(pKey, Lifecycle.stable(), pBootstrap);
   }
   ...
   private static <T> DefaultedRegistry<T> registerDefaulted(ResourceKey<? extends Registry<T>> pKey, String pDefaultKey, BuiltInRegistries.RegistryBootstrap<T> pBootstrap) {
      return registerDefaulted(pKey, pDefaultKey, Lifecycle.stable(), pBootstrap);
   }
   ...
   private static <T> DefaultedRegistry<T> registerDefaultedWithIntrusiveHolders(ResourceKey<? extends Registry<T>> pKey, String pDefaultKey, BuiltInRegistries.RegistryBootstrap<T> pBootstrap) {
      return registerDefaultedWithIntrusiveHolders(pKey, pDefaultKey, Lifecycle.stable(), pBootstrap);
   }

   private static <T> Registry<T> registerSimple(ResourceKey<? extends Registry<T>> pKey, Lifecycle pLifecycle, BuiltInRegistries.RegistryBootstrap<T> pBootstrap) {
      return internalRegister(pKey, new MappedRegistry<>(pKey, pLifecycle, false), pBootstrap, pLifecycle);
   }

   private static <T> DefaultedRegistry<T> registerDefaulted(ResourceKey<? extends Registry<T>> pKey, String pDefaultKey, Lifecycle pLifecycle, BuiltInRegistries.RegistryBootstrap<T> pBootstrap) {
      return internalRegister(pKey, new DefaultedMappedRegistry<>(pDefaultKey, pKey, pLifecycle, false), pBootstrap, pLifecycle);
   }

   private static <T> DefaultedRegistry<T> registerDefaultedWithIntrusiveHolders(ResourceKey<? extends Registry<T>> pKey, String pDefaultKey, Lifecycle pLifecycle, BuiltInRegistries.RegistryBootstrap<T> pBootstrap) {
      return internalRegister(pKey, new DefaultedMappedRegistry<>(pDefaultKey, pKey, pLifecycle, true), pBootstrap, pLifecycle);
   }

   private static <T, R extends WritableRegistry<T>> R internalRegister(ResourceKey<? extends Registry<T>> pKey, R pRegistry, BuiltInRegistries.RegistryBootstrap<T> pBootstrap, Lifecycle pLifecycle) {
   ...
   private static <T extends Registry<?>> void validate(Registry<T> pRegistry) {
      pRegistry.forEach((p_259410_) -> {
         if (p_259410_.keySet().isEmpty()) {
            Util.logAndPauseIfInIde("Registry '" + pRegistry.getKey(p_259410_) + "' was empty after loading");
         }
