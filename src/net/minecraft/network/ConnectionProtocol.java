...
public enum ConnectionProtocol implements BundlerInfo.Provider {
   ...
   public int getPacketId(PacketFlow pFlow, Packet<?> pPacket) {
      return this.flows.get(pFlow).getId(pPacket.getClass());
   }

   public BundlerInfo getBundlerInfo(PacketFlow pFlow) {
      return this.flows.get(pFlow).bundlerInfo();
   }
   ...
   static class PacketSet<T extends PacketListener> {
      ...
      public <P extends BundlePacket<T>> ConnectionProtocol.PacketSet<T> withBundlePacket(Class<P> pPacketClass, Function<Iterable<Packet<T>>, P> pBundler) {
         if (this.bundlerInfo != BundlerInfo.EMPTY) {
            ...
            this.bundlerInfo = BundlerInfo.createForPacket(pPacketClass, pBundler, bundledelimiterpacket);
            this.extraClasses.add(pPacketClass);
            return this;
         }
      }

      public int getId(Class<?> pPacketClass) {
         return this.classToId.getInt(pPacketClass);
      }
      ...
      public void listAllPackets(Consumer<Class<? extends Packet<?>>> pConsumer) {
         this.classToId.keySet().stream().filter((p_264721_) -> {
            return p_264721_ != (Object)BundleDelimiterPacket.class;
         }).forEach(pConsumer);
         this.extraClasses.forEach(pConsumer);
      }
