...
public class VanillaHusbandryAdvancements implements AdvancementSubProvider {
   ...
   public void generate(HolderLookup.Provider pRegistries, Consumer<Advancement> pWriter) {
      ...
      Advancement advancement2 = Advancement.Builder.advancement().parent(advancement).display(Items.WHEAT, Component.translatable("advancements.husbandry.breed_an_animal.title"), Component.translatable("advancements.husbandry.breed_an_animal.description"), (ResourceLocation)null, FrameType.TASK, true, true, false).requirements(RequirementsStrategy.OR).addCriterion("bred", BredAnimalsTrigger.TriggerInstance.bredAnimals()).save(pWriter, "husbandry/breed_an_animal");
      ...
      Advancement advancement5 = addFishBuckets(Advancement.Builder.advancement()).parent(advancement4).requirements(RequirementsStrategy.OR).display(Items.PUFFERFISH_BUCKET, Component.translatable("advancements.husbandry.tactical_fishing.title"), Component.translatable("advancements.husbandry.tactical_fishing.description"), (ResourceLocation)null, FrameType.TASK, true, true, false).save(pWriter, "husbandry/tactical_fishing");
      ...
      Advancement.Builder.advancement().parent(advancement).display(Items.OAK_BOAT, Component.translatable("advancements.husbandry.ride_a_boat_with_a_goat.title"), Component.translatable("advancements.husbandry.ride_a_boat_with_a_goat.description"), (ResourceLocation)null, FrameType.TASK, true, true, false).addCriterion("ride_a_boat_with_a_goat", StartRidingTrigger.TriggerInstance.playerStartsRiding(EntityPredicate.Builder.entity().vehicle(EntityPredicate.Builder.entity().of(EntityType.BOAT).passenger(EntityPredicate.Builder.entity().of(EntityType.GOAT).build()).build()))).save(pWriter, "husbandry/ride_a_boat_with_a_goat");
      ...
      Advancement advancement13 = Advancement.Builder.advancement().parent(advancement12).display(Items.TORCHFLOWER_SEEDS, Component.translatable("advancements.husbandry.feed_snifflet.title"), Component.translatable("advancements.husbandry.feed_snifflet.description"), (ResourceLocation)null, FrameType.TASK, true, true, true).addCriterion("feed_snifflet", PlayerInteractTrigger.TriggerInstance.itemUsedOnEntity(ItemPredicate.Builder.item().of(ItemTags.SNIFFER_FOOD), EntityPredicate.wrap(EntityPredicate.Builder.entity().of(EntityType.SNIFFER).flags(EntityFlagsPredicate.Builder.flags().setIsBaby(true).build()).build()))).save(pWriter, "husbandry/feed_snifflet");
   ...
   public static Advancement createBreedAllAnimalsAdvancement(Advancement pParent, Consumer<Advancement> pWriter, Stream<EntityType<?>> pBreedableAnimals, Stream<EntityType<?>> pIndirectlyBreedableAnimals) {
   ...
   private static Advancement.Builder addLeashedFrogVariants(Advancement.Builder pBuilder) {
      BuiltInRegistries.FROG_VARIANT.holders().forEach((p_286193_) -> {
         pBuilder.addCriterion(p_286193_.key().location().toString(), PlayerInteractTrigger.TriggerInstance.itemUsedOnEntity(ItemPredicate.Builder.item().of(Items.LEAD), EntityPredicate.wrap(EntityPredicate.Builder.entity().of(EntityType.FROG).subPredicate(EntitySubPredicate.variant(p_286193_.value())).build())));
      });
      return pBuilder;
   }
   ...
   private static Advancement.Builder addBreedable(Advancement.Builder pBuilder, Stream<EntityType<?>> pBreedableAnimals, Stream<EntityType<?>> pIndirectlyBreedableAnimals) {
      pBreedableAnimals.forEach((p_266621_) -> {
         pBuilder.addCriterion(EntityType.getKey(p_266621_).toString(), BredAnimalsTrigger.TriggerInstance.bredAnimals(EntityPredicate.Builder.entity().of(p_266621_)));
      });
      pIndirectlyBreedableAnimals.forEach((p_266619_) -> {
         pBuilder.addCriterion(EntityType.getKey(p_266619_).toString(), BredAnimalsTrigger.TriggerInstance.bredAnimals(EntityPredicate.Builder.entity().of(p_266619_).build(), EntityPredicate.Builder.entity().of(p_266619_).build(), EntityPredicate.ANY));
      });
      return pBuilder;
   }
   ...
   private static Advancement.Builder addFishBuckets(Advancement.Builder pBuilder) {
      for(Item item : FISH_BUCKETS) {
         pBuilder.addCriterion(BuiltInRegistries.ITEM.getKey(item).getPath(), FilledBucketTrigger.TriggerInstance.filledBucket(ItemPredicate.Builder.item().of(item).build()));
      }
   ...
   private static Advancement.Builder addFish(Advancement.Builder pBuilder) {
      for(Item item : FISH) {
         pBuilder.addCriterion(BuiltInRegistries.ITEM.getKey(item).getPath(), FishingRodHookedTrigger.TriggerInstance.fishedItem(ItemPredicate.ANY, EntityPredicate.ANY, ItemPredicate.Builder.item().of(item).build()));
      }
   ...
   private static Advancement.Builder addCatVariants(Advancement.Builder pBuilder) {
      BuiltInRegistries.CAT_VARIANT.entrySet().stream().sorted(Entry.comparingByKey(Comparator.comparing(ResourceKey::location))).forEach((p_249721_) -> {
         pBuilder.addCriterion(p_249721_.getKey().location().toString(), TameAnimalTrigger.TriggerInstance.tamedAnimal(EntityPredicate.Builder.entity().subPredicate(EntitySubPredicate.variant(p_249721_.getValue())).build()));
      });
      return pBuilder;
   }
}