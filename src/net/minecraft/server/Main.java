...
public class Main {
   ...
   public static void main(String[] pArgs) {
      ...
      try {
         ...
         final DedicatedServer dedicatedserver = MinecraftServer.spin((p_236710_) -> {
            DedicatedServer dedicatedserver1 = new DedicatedServer(p_236710_, levelstoragesource$levelstorageaccess, packrepository, worldstem, dedicatedserversettings, DataFixers.getDataFixer(), services, LoggerChunkProgressListener::new);
   ...
   private static WorldLoader.InitConfig loadOrCreateConfig(DedicatedServerProperties pServerProperties, LevelStorageSource.LevelStorageAccess pLevelStorage, boolean pSafeMode, PackRepository pPackRepository) {
      ...
      if (worlddataconfiguration != null) {
         ...
         flag = true;
         worlddataconfiguration1 = new WorldDataConfiguration(pServerProperties.initialDataPackConfiguration, FeatureFlags.DEFAULT_FLAGS);
      }

      WorldLoader.PackConfig worldloader$packconfig = new WorldLoader.PackConfig(pPackRepository, worlddataconfiguration1, pSafeMode, flag);
      return new WorldLoader.InitConfig(worldloader$packconfig, Commands.CommandSelection.DEDICATED, pServerProperties.functionPermissionLevel);
   }

   private static void forceUpgrade(LevelStorageSource.LevelStorageAccess pLevelStorage, DataFixer pDataFixer, boolean pEraseCache, BooleanSupplier pUpgradeWorld, Registry<LevelStem> pDimesions) {
      LOGGER.info("Forcing world upgrade!");
      WorldUpgrader worldupgrader = new WorldUpgrader(pLevelStorage, pDataFixer, pDimesions, pEraseCache);
