...
public class LootItemConditions {
   ...
   private static LootItemConditionType register(String pRegistryName, Serializer<? extends LootItemCondition> pSerializer) {
      return Registry.register(BuiltInRegistries.LOOT_CONDITION_TYPE, new ResourceLocation(pRegistryName), new LootItemConditionType(pSerializer));
   }
   ...
   public static <T> Predicate<T> andConditions(Predicate<T>[] pConditions) {
      ...
      switch (pConditions.length) {
         case 0:
            ...
            break;
         case 1:
            predicate = pConditions[0];
            break;
         case 2:
            predicate = pConditions[0].and(pConditions[1]);
            break;
         default:
            predicate = (p_81845_) -> {
               for(Predicate<T> predicate1 : pConditions) {
   ...
   public static <T> Predicate<T> orConditions(Predicate<T>[] pConditions) {
      ...
      switch (pConditions.length) {
         ...
         case 1:
            ...
            break;
         case 2:
            predicate = pConditions[0].or(pConditions[1]);
            break;
         default:
            predicate = (p_81838_) -> {
               for(Predicate<T> predicate1 : pConditions) {
