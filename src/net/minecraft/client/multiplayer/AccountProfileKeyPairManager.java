...
public class AccountProfileKeyPairManager implements ProfileKeyPairManager {
   ...
   private void writeProfileKeyPair(@Nullable ProfileKeyPair pProfileKeyPair) {
      ...
      if (pProfileKeyPair != null) {
         if (SharedConstants.IS_RUNNING_IN_IDE) {
            ProfileKeyPair.CODEC.encodeStart(JsonOps.INSTANCE, pProfileKeyPair).result().ifPresent((p_254406_) -> {
               try {
   ...
   private static ProfilePublicKey.Data parsePublicKey(KeyPairResponse pKeyPairResponse) throws CryptException {
      if (!Strings.isNullOrEmpty(pKeyPairResponse.getPublicKey()) && pKeyPairResponse.getPublicKeySignature() != null && pKeyPairResponse.getPublicKeySignature().array().length != 0) {
         try {
            Instant instant = Instant.parse(pKeyPairResponse.getExpiresAt());
            PublicKey publickey = Crypt.stringToRsaPublicKey(pKeyPairResponse.getPublicKey());
            ByteBuffer bytebuffer = pKeyPairResponse.getPublicKeySignature();
