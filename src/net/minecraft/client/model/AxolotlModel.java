...
public class AxolotlModel<T extends Axolotl & LerpingModel> extends AgeableListModel<T> {
   ...
   public void setupAnim(T pEntity, float pLimbSwing, float pLimbSwingAmount, float pAgeInTicks, float pNetHeadYaw, float pHeadPitch) {
      ...
      if (pEntity.isPlayingDead()) {
         ...
         boolean flag = pLimbSwingAmount > 1.0E-5F || pEntity.getXRot() != pEntity.xRotO || pEntity.getYRot() != pEntity.yRotO;
   ...
   private void setupLayStillOnGroundAnimation(float pAgeInTicks, float pNetHeadYaw) {
      ...
      this.body.yRot = this.lerpTo(this.body.yRot, pNetHeadYaw * ((float)Math.PI / 180F));
   ...
   private void setupSwimmingAnimation(float pAgeInTicks, float pHeadPitch) {
      ...
      float f2 = Mth.cos(f);
      float f3 = 0.13F * f1;
      this.body.xRot = this.lerpTo(0.1F, this.body.xRot, pHeadPitch * ((float)Math.PI / 180F) + f3);
