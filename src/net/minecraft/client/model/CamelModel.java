...
public class CamelModel<T extends Camel> extends HierarchicalModel<T> {
   ...
   public void setupAnim(T pEntity, float pLimbSwing, float pLimbSwingAmount, float pAgeInTicks, float pNetHeadYaw, float pHeadPitch) {
      ...
      this.toggleInvisibleParts(pEntity);
      this.animateWalk(CamelAnimation.CAMEL_WALK, pLimbSwing, pLimbSwingAmount, 2.0F, 2.5F);
      this.animate(pEntity.sitAnimationState, CamelAnimation.CAMEL_SIT, pAgeInTicks, 1.0F);
      this.animate(pEntity.sitPoseAnimationState, CamelAnimation.CAMEL_SIT_POSE, pAgeInTicks, 1.0F);
      this.animate(pEntity.sitUpAnimationState, CamelAnimation.CAMEL_STANDUP, pAgeInTicks, 1.0F);
      this.animate(pEntity.idleAnimationState, CamelAnimation.CAMEL_IDLE, pAgeInTicks, 1.0F);
      this.animate(pEntity.dashAnimationState, CamelAnimation.CAMEL_DASH, pAgeInTicks, 1.0F);
   }

   private void applyHeadRotation(T pEntity, float pNetHeadYaw, float pHeadPitch, float pAgeInTicks) {
      pNetHeadYaw = Mth.clamp(pNetHeadYaw, -30.0F, 30.0F);
      pHeadPitch = Mth.clamp(pHeadPitch, -25.0F, 45.0F);
      if (pEntity.getJumpCooldown() > 0) {
         float f = pAgeInTicks - (float)pEntity.tickCount;
         float f1 = 45.0F * ((float)pEntity.getJumpCooldown() - f) / 55.0F;
         pHeadPitch = Mth.clamp(pHeadPitch + f1, -25.0F, 70.0F);
      }
   ...
   private void toggleInvisibleParts(T pEntity) {
