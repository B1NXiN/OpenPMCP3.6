...
public class OnlineOptionsScreen extends SimpleOptionsSubScreen {
   ...
   public static OnlineOptionsScreen createOnlineOptionsScreen(Minecraft pMinecraft, Screen pLastScreen, Options pSmallOptions) {
      ...
      OptionInstance<Unit> optioninstance = Optionull.map(pMinecraft.level, (p_287799_0_) -> {
         Difficulty difficulty = p_287799_0_.getDifficulty();
         return new OptionInstance<>("options.difficulty.online", OptionInstance.noTooltip(), (p_260895_1_, p_260895_2_) -> {
            return difficulty.getDisplayName();
         }, new OptionInstance.Enum<>(List.of(Unit.INSTANCE), Codec.EMPTY.codec()), Unit.INSTANCE, (p_261325_0_) -> {
         });
      });
