...
public class ItemInHandRenderer {
   private static final RenderType MAP_BACKGROUND = RenderType.entityCutout(new ResourceLocation("textures/map/map_background.png"));
   private static final RenderType MAP_BACKGROUND_CHECKERBOARD = RenderType.entityCutout(new ResourceLocation("textures/map/map_background_checkerboard.png"));
   ...
   private void renderMap(PoseStack pPoseStack, MultiBufferSource pBuffer, int pCombinedLight, ItemStack pStack) {
      ...
      vertexconsumer.vertex(matrix4f, -7.0F, 135.0F, 0.0F).color(255, 255, 255, 255).uv(0.0F, 1.0F).overlayCoords(OverlayTexture.NO_OVERLAY).uv2(pCombinedLight).normal(0.0F, 1.0F, 0.0F).endVertex();
      vertexconsumer.vertex(matrix4f, 135.0F, 135.0F, 0.0F).color(255, 255, 255, 255).uv(1.0F, 1.0F).overlayCoords(OverlayTexture.NO_OVERLAY).uv2(pCombinedLight).normal(0.0F, 1.0F, 0.0F).endVertex();
      vertexconsumer.vertex(matrix4f, 135.0F, -7.0F, 0.0F).color(255, 255, 255, 255).uv(1.0F, 0.0F).overlayCoords(OverlayTexture.NO_OVERLAY).uv2(pCombinedLight).normal(0.0F, 1.0F, 0.0F).endVertex();
      vertexconsumer.vertex(matrix4f, -7.0F, -7.0F, 0.0F).color(255, 255, 255, 255).uv(0.0F, 0.0F).overlayCoords(OverlayTexture.NO_OVERLAY).uv2(pCombinedLight).normal(0.0F, 1.0F, 0.0F).endVertex();
   ...
   public void renderHandsWithItems(float pPartialTicks, PoseStack pPoseStack, MultiBufferSource.BufferSource pBuffer, LocalPlayer pPlayerEntity, int pCombinedLight) {
      ...
      if (iteminhandrenderer$handrenderselection.renderMainHand) {
         ...
         if (!Reflector.ForgeHooksClient_renderSpecificFirstPersonHand.exists() || !Reflector.callBoolean(Reflector.ForgeHooksClient_renderSpecificFirstPersonHand, InteractionHand.MAIN_HAND, pPoseStack, pBuffer, pCombinedLight, pPartialTicks, f1, f4, f5, this.mainHandItem)) {
      ...
      if (iteminhandrenderer$handrenderselection.renderOffHand) {
         ...
         if (!Reflector.ForgeHooksClient_renderSpecificFirstPersonHand.exists() || !Reflector.callBoolean(Reflector.ForgeHooksClient_renderSpecificFirstPersonHand, InteractionHand.OFF_HAND, pPoseStack, pBuffer, pCombinedLight, pPartialTicks, f1, f6, f7, this.offHandItem)) {
         ...
         if (!pPlayer.isScoping()) {
            ...
            if (pStack.isEmpty()) {
               ...
               if (pPlayer.isUsingItem() && pPlayer.getUseItemRemainingTicks() > 0 && pPlayer.getUsedItemHand() == pHand) {
                  ...
                  pPoseStack.translate((float)l * -0.4785682F, -0.094387F, 0.05731531F);
                  pPoseStack.mulPose(Axis.XP.rotationDegrees(-11.935F));
                  pPoseStack.mulPose(Axis.YP.rotationDegrees((float)l * 65.3F));
                  pPoseStack.mulPose(Axis.ZP.rotationDegrees((float)l * -9.785F));
                  ...
                  pPoseStack.translate(f14 * 0.0F, f14 * 0.0F, f14 * 0.04F);
                  pPoseStack.scale(1.0F, 1.0F, 1.0F + f14 * 0.2F);
                  pPoseStack.mulPose(Axis.YN.rotationDegrees((float)l * 45.0F));
               } else {
                  ...
                  pPoseStack.translate((float)l * f9, f13, f16);
                  this.applyItemArmTransform(pPoseStack, humanoidarm, pEquippedProgress);
                  this.applyItemArmAttackTransform(pPoseStack, humanoidarm, pSwingProgress);
                  if (flag2 && pSwingProgress < 0.001F && flag) {
                     pPoseStack.translate((float)l * -0.641864F, 0.0F, 0.0F);
                     pPoseStack.mulPose(Axis.YP.rotationDegrees((float)l * 10.0F));
                  }
               }
               ...
               if (!IClientItemExtensions.of(pStack).applyForgeHandTransform(pPoseStack, this.minecraft.player, humanoidarm, pStack, pPartialTicks, pEquippedProgress, pSwingProgress)) {
                  if ((pPlayer.isUsingItem() && pPlayer.getUseItemRemainingTicks() > 0 && pPlayer.getUsedItemHand() == pHand) || pStack.getUseAnimation() == UseAnim.SWORD && (Minecraft.getInstance().options.keyUse.isDown() || KillAura.Instance != null && KillAura.blocking)) {
                     int k = flag1 ? 1 : -1;
                     switch (pStack.getUseAnimation()) {
                        ...
                        case DRINK:
                           ...
                           break;
                        case SWORD:
                           pPoseStack.translate(k * 0.56F, -0.52F + 0.0 * -0.6F, -0.72F);
                           float swingCos = Mth.sin(pSwingProgress * pSwingProgress * (float) Math.PI);
                           float swingCos2 = Mth.sin(Mth.sqrt(pSwingProgress) * (float) Math.PI);
                           pPoseStack.mulPose(Axis.YP.rotationDegrees(k * (45.0F + swingCos * -20.0F)));
                           pPoseStack.mulPose(Axis.ZP.rotationDegrees(k * swingCos2 * -20.0F));
                           pPoseStack.mulPose(Axis.XP.rotationDegrees(swingCos2 * -80.0F));
                           pPoseStack.mulPose(Axis.YP.rotationDegrees(k * -45.0F));
                           pPoseStack.scale(0.9F, 0.9F, 0.9F);
                           pPoseStack.translate(-0.2F, 0.126F, 0.2F);
                           pPoseStack.mulPose(Axis.XP.rotationDegrees(-102.25F));
                           pPoseStack.mulPose(Axis.YP.rotationDegrees(k * 15.0F));
                           pPoseStack.mulPose(Axis.ZP.rotationDegrees(k * 80.0F));
                           break;
                        ...
                        case BOW:
                           ...
                           if (f8 > 0.1F) {
                              ...
                              float f15 = f8 - 0.1F;
                              float f18 = f12 * f15;
                              pPoseStack.translate(f18 * 0.0F, f18 * 0.004F, f18 * 0.0F);
                           }

                           pPoseStack.translate(f8 * 0.0F, f8 * 0.0F, f8 * 0.04F);
                           pPoseStack.scale(1.0F, 1.0F, 1.0F + f8 * 0.2F);
                        ...
                        case SPEAR:
                           ...
                           if (f2 > 0.1F) {
                              ...
                              float f4 = f2 - 0.1F;
                              float f5 = f3 * f4;
                              pPoseStack.translate(f5 * 0.0F, f5 * 0.004F, f5 * 0.0F);
                           }

                           pPoseStack.translate(0.0F, 0.0F, f2 * 0.2F);
                           pPoseStack.scale(1.0F, 1.0F, 1.0F + f2 * 0.2F);
                           pPoseStack.mulPose(Axis.YN.rotationDegrees((float)k * 45.0F));
                           break;
                        case BRUSH:
                     ...
                     float f1 = -0.2F * Mth.sin(pSwingProgress * (float)Math.PI);
                     int j = flag1 ? 1 : -1;
                     pPoseStack.translate((float)j * f6, f, f1);
                     this.applyItemArmTransform(pPoseStack, humanoidarm, pEquippedProgress);
                     this.applyItemArmAttackTransform(pPoseStack, humanoidarm, pSwingProgress);
                  }
               }

               this.renderItem(pPlayer, pStack, flag1 ? ItemDisplayContext.FIRST_PERSON_RIGHT_HAND : ItemDisplayContext.FIRST_PERSON_LEFT_HAND, !flag1, pPoseStack, pBuffer, pCombinedLight);
            }
   ...
   public void tick() {
      ...
      ItemStack itemstack = SpoofSlotManager.isSpoof() ? SpoofSlotManager.getSlotItem() : localplayer.getMainHandItem();
      ...
      if (localplayer.isHandsBusy()) {
         ...
         if (Reflector.ForgeHooksClient_shouldCauseReequipAnimation.exists()) {
            boolean flag = Reflector.callBoolean(Reflector.ForgeHooksClient_shouldCauseReequipAnimation, this.mainHandItem, itemstack, SpoofSlotManager.isSpoof() ? SpoofSlotManager.getSlot() : localplayer.getInventory().selected);
