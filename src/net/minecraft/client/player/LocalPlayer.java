...
public class LocalPlayer extends AbstractClientPlayer {
   ...
   public void tick() {
      if (this.level().hasChunkAt(this.getBlockX(), this.getBlockZ())) {
         EventManager.instance.call(new EventUpdate());
         super.tick();
         if (this.isPassenger()) {
      ...
      EventManager.instance.call(new EventMotion(EventMotion.Type.Post, getX(), getY(), getZ(), onGround()));
   }
   ...
   private void sendPosition() {
      EventMotion motion = new EventMotion(EventMotion.Type.Pre, this.getYRot(), this.getXRot(), getX(), getY(), getZ(), onGround());
      ...
      if (this.isControlledCamera()) {
         double d4 = motion.getX() - this.xLast;
         double d0 = motion.getY() - this.yLast1;
         double d1 = motion.getZ() - this.zLast;
         double d2 = (double)(motion.getYaw() - this.yRotLast);
         double d3 = (double)(motion.getPitch() - this.xRotLast);
         ...
         if (this.isPassenger()) {
            ...
            this.connection.send(new ServerboundMovePlayerPacket.PosRot(vec3.x, -999.0D, vec3.z, motion.getYaw(), motion.getPitch(), motion.isOnGround()));
            flag1 = false;
         } else if (flag1 && flag2) {
            this.connection.send(new ServerboundMovePlayerPacket.PosRot(motion.getX(), motion.getY(), motion.getZ(), motion.getYaw(), motion.getPitch(), motion.isOnGround()));
         } else if (flag1) {
            this.connection.send(new ServerboundMovePlayerPacket.Pos(motion.getX(), motion.getY(), motion.getZ(), motion.isOnGround()));
         } else if (flag2) {
            this.connection.send(new ServerboundMovePlayerPacket.Rot(motion.getYaw(), motion.getPitch(), motion.isOnGround()));
         } else if (this.lastOnGround != motion.isOnGround()){
            this.connection.send(new ServerboundMovePlayerPacket.StatusOnly(motion.isOnGround()));
         }

         ++this.postTicks;

         if (flag1) {
            this.xLast = motion.getX();
            this.yLast1 = motion.getY();
            this.zLast = motion.getZ();
            this.positionReminder = 0;
            this.postTicks = 0;
         }

         if (flag2) {
            this.yRotLast = motion.getYaw();
            this.xRotLast = motion.getPitch();
         }

         this.lastOnGround = motion.isOnGround();
   ...
   public void aiStep() {
      ...
      if (this.isUsingItem() && !this.isPassenger()) {
         EventSlow slow = new EventSlow(0.2F,0.2F);
      ...
      Sprint sprint = (Sprint) PMCP.Instance.getMode(Sprint.class);

      if (sprint.isEnable()) {
         boolean isMove = this.input.hasForwardImpulse() && ((float)this.getFoodData().getFoodLevel() > 6.0F && !sprint.fullFoodLevel.getValue() || this.getAbilities().mayfly) && this.isMoving();
         boolean isSilent = SilentRotation.targetRotation != null && (SilentRotation.getCategory() == MoveCategory.Silent || SilentRotation.getCategory() == MoveCategory.Auto);
         boolean noRot = SilentRotation.targetRotation == null || Math.abs(RotationUtils.getAngleDifference(SilentRotation.getMovingYaw(), SilentRotation.targetRotation.getYaw())) < 67.0f;
         ...
         if (isSilent && !noRot || ((this.isUsingItem() && !sprint.food.getValue()) || this.isPassenger() || flag8)) {
            this.setSprinting(false);
         } else {
            if ((!this.isInWater() || this.isUnderWater())) this.setSprinting(isMove);
         }
      } else {
   ...
   public void move(MoverType pType, Vec3 pPos) {
      ...
      double d0 = this.getX();
      double d1 = this.getZ();
      EventManager.instance.call(move);
   ...
   public Rotation getRotations() {
      return new Rotation(this.getYRot(), this.getXRot());
   }
}