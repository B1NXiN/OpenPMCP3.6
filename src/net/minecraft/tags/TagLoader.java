...
public class TagLoader<T> {
   ...
   public TagLoader(Function<ResourceLocation, Optional<? extends T>> pIdToValue, String pDirectory) {
   ...
   private Either<Collection<TagLoader.EntryWithSource>, Collection<T>> build(TagEntry.Lookup<T> pLookup, List<TagLoader.EntryWithSource> pEntries) {
      ...
      for(TagLoader.EntryWithSource tagloader$entrywithsource : pEntries) {
         if (!tagloader$entrywithsource.entry().build(pLookup, builder::add)) {
   ...
   public Map<ResourceLocation, Collection<T>> build(Map<ResourceLocation, List<TagLoader.EntryWithSource>> pBuilders) {
      ...
      pBuilders.forEach((p_284685_, p_284686_) -> {
