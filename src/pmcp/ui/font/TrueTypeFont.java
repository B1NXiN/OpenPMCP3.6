...
import pmcp.utils.render.GLUtils;
...
public class TrueTypeFont implements AutoCloseable {
    ...
    public float drawComponent(PoseStack matrices, Component comp, float x, float y, int defaultColor) {
        return drawComponent(matrices, comp, x, y, defaultColor, false);
    }

    public float drawComponentWithShadow(PoseStack matrices, Component comp, float x, float y, int defaultColor) {
        return drawComponent(matrices, comp, x, y, defaultColor, true);
    }

    /** 用你现有的字符串渲染管线渲染 Component（把每段 Style 转成等价的 § 控制码） */
    public float drawComponent(PoseStack matrices, Component comp, float x, float y, int defaultColor, boolean dropShadow) {
        if (comp == null) return x;

        String legacy = componentToLegacyString(comp, defaultColor);
        return drawString(matrices, legacy, x, y, defaultColor, dropShadow);
    }

    /** 计算 Component 的显示宽度（按你当前字体/样式开关） */
    public float getComponentWidth(Component comp) {
        if (comp == null) return 0f;
        String legacy = componentToLegacyString(comp, 0xFFFFFFFF);
        return getStringWidth(legacy);
    }
    ...
    /** 把 Component 展成含 § 控制码的字符串，交给现有 renderString 解析 */
    private String componentToLegacyString(Component comp, int defaultColor) {
        StringBuilder out = new StringBuilder();
        final Style[] current = { Style.EMPTY }; // 当前样式，用于和下一段对比减少多余控制码

        // 把所有可见文本段收集出来（text + style）
        java.util.List<StyledChunk> chunks = collectChunks(comp);

        // 上次写入的“启用状态”
        boolean lastBold = false, lastItalic = false, lastUnder = false, lastStrike = false, lastObf = false;
        int lastColorIdx = -1; // -1 表示沿用 defaultColor

        for (StyledChunk ch : chunks) {
            Style s = ch.style;

            // ---- 颜色 ----
            int colorIdx = -1;
            if (s.getColor() != null) {
                colorIdx = toLegacyColorIndex(s.getColor());
            } else if (lastColorIdx == -1) {
                // 第一段没有指定颜色就沿用 defaultColor（不发颜色码）
                colorIdx = -1;
            }

            // 如果颜色变化，先发 §r 复位再重新套样式，避免残留
            boolean colorChanged = colorIdx != lastColorIdx;
            if (colorChanged) {
                out.append('§').append('r'); // reset
                lastBold = lastItalic = lastUnder = lastStrike = lastObf = false;
            }

            if (colorIdx >= 0) {
                out.append('§').append("0123456789abcdef".charAt(colorIdx));
            }

            // ---- 样式（按 MC 约定开启即可）----
            if (s.isBold() && !lastBold) out.append('§').append('l');
            if (s.isItalic() && !lastItalic) out.append('§').append('o');
            if (s.isUnderlined() && !lastUnder) out.append('§').append('n');
            if (s.isStrikethrough() && !lastStrike) out.append('§').append('m');
            if (s.isObfuscated() && !lastObf) out.append('§').append('k');
            ...
            lastBold = s.isBold();
            lastItalic = s.isItalic();
            lastUnder = s.isUnderlined();
            lastStrike = s.isStrikethrough();
            lastObf = s.isObfuscated();
        }

        // 末尾 reset（可选，不影响下一次 drawString，因为你每次都会传默认颜色）
        // out.append('§').append('r');

        return out.toString();
    }

    private int toLegacyColorIndex(TextColor color) {
        ...
        for (int i = 0; i < 16; i++) {
            int base = COLOR_TABLE[i] & 0xFFFFFF;
            int dr = ((rgb >> 16) & 0xFF) - ((base >> 16) & 0xFF);
            int dg = ((rgb >> 8) & 0xFF) - ((base >> 8) & 0xFF);
            int db = (rgb & 0xFF) - (base & 0xFF);
    ...
    /** 把 Component 递归展开为 (text, style) 段 */
    private java.util.List<StyledChunk> collectChunks(FormattedText text) {
        java.util.List<StyledChunk> out = new ArrayList<>();

        text.visit((style, str) -> {
            if (str != null && !str.isEmpty()) {
                out.add(new StyledChunk(VIPUtils.replaceVIPChars(str), style));
            }
    ...
    private static class StyledChunk {
        final String text;
        final Style style;
        StyledChunk(String t, Style s) { this.text = t; this.style = s == null ? Style.EMPTY : s; }
    }
