...
public abstract class Animation {
    ...
    public double getLinearOutput() {
        return 1.0 - (double) this.timerUtil.getTimePassed() / (double) this.duration * this.endPoint;
    }
    ...
    public boolean isDone() {
        return this.timerUtil.hasTimePassed(this.duration);
    }
    ...
    public Animation setDirection(Direction direction) {
        if (this.direction != direction) {
            this.direction = direction;
            this.timerUtil.setLastTime(System.currentTimeMillis() - ((long) this.duration - Math.min(this.duration, this.timerUtil.getTimePassed())));
        }
        return this;
    }
    ...
    public double getOutput() {
        if (this.direction.forwards()) {
            ...
            return this.getEquation((double) this.timerUtil.getTimePassed() / (double) this.duration) * this.endPoint;
        }
        if (this.isDone()) {
            return 0.0;
        }
        if (this.correctOutput()) {
            double revTime = Math.min(this.duration, Math.max(0L, (long) this.duration - this.timerUtil.getTimePassed()));
            return this.getEquation(revTime / (double) this.duration) * this.endPoint;
        }
        return (1.0 - this.getEquation((double) this.timerUtil.getTimePassed() / (double) this.duration)) * this.endPoint;
    }
